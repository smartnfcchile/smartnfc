...
export async function GET(request: Request, { params }: { params: Promise<{ designId: string }> }) {
  ...
  const qr = `https://api.qrserver.com/v1/create-qr-code/?format=svg&size=1200x1200&margin=48&data=${encodeURIComponent(publicUrl)}`;
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="91.6mm" height="59.98mm" viewBox="0 0 916 600"><rect width="916" height="600" fill="${esc(side.background)}"/><rect y="550" width="916" height="50" fill="${esc(side.primary)}"/><rect x="78" y="110" width="90" height="8" rx="4" fill="${esc(side.secondary)}"/><text x="78" y="255" fill="${esc(side.text)}" font-family="Arial,sans-serif" font-size="52" font-weight="700">${esc(side.name || side.company)}</text><text x="78" y="302" fill="${esc(side.muted)}" font-family="Arial,sans-serif" font-size="25">${esc([side.position, side.company].filter(Boolean).join(" · "))}</text><text x="78" y="365" fill="${esc(side.text)}" font-family="Arial,sans-serif" font-size="23">${esc(side.tagline)}</text>${side.qrVisible ? `<rect x="655" y="305" width="205" height="205" rx="12" fill="white"/><image x="670" y="320" width="175" height="175" xlink:href="${esc(qr)}"/>` : ""}${side.nfcVisible ? `<text x="650" y="115" fill="${esc(side.text)}" font-family="Arial,sans-serif" font-size="18">◖))) ${esc(side.nfcText)}</text>` : ""}</svg>`;
