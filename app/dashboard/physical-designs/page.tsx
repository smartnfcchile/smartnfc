...
export default async function PhysicalDesignsPage({ searchParams }: { searchParams: Promise<{ cardId?: string }> }) {
  ...
  return <div className="space-y-6">
    <header className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
      <div><p className="text-xs font-bold uppercase tracking-[.22em] text-blue-700 dark:text-blue-400">Producción SmartNFC</p><h1 className="mt-1 text-3xl font-bold text-slate-950 dark:text-white">Diseños físicos</h1><p className="mt-1 text-sm text-slate-600 dark:text-slate-400">Diseña anverso y reverso con parámetros seguros para impresión CR80.</p></div>
      <Link href="/dashboard/physical-designs/orders" className="rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 px-4 py-2.5 text-sm font-semibold text-slate-900 dark:text-white hover:border-blue-500">Historial de pedidos</Link>
    </header>
