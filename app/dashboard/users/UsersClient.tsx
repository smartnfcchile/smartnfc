...
import { createCollaboratorWithCard, resendInvitationFromDashboardAction, suspendCollaboratorUser } from "./actions";
...
export default function UsersClient({ initialUsers, currentUserId }: Props) {
  ...
  const handleSuspend = async (userId: string, userName: string) => {
    if (!window.confirm(`¿Suspender al colaborador "${userName}"?\n\nPerderá el acceso y sus tarjetas dejarán de estar activas. Su historial y sus prospectos del CRM se conservarán.`)) return;
    try { await suspendCollaboratorUser(userId); router.refresh(); }
    catch (err: unknown) { alert(err instanceof Error ? err.message : "No fue posible suspender al colaborador."); }
  };
  ...
  return <div className="space-y-6">
    ...
    <div className="bg-white dark:bg-slate-900/40 border border-slate-200 dark:border-slate-800 rounded-2xl overflow-hidden shadow-sm"><div className="overflow-x-auto"><table className="w-full text-left border-collapse"><thead><tr className="border-b border-slate-200 dark:border-slate-800 text-[10px] uppercase tracking-wider text-slate-700 dark:text-slate-400 bg-slate-100 dark:bg-slate-950/20"><th className="py-3.5 px-4">Integrante</th><th className="py-3.5 px-4">Correo</th><th className="py-3.5 px-4 text-center">Rol</th><th className="py-3.5 px-4 text-center">Tarjetas</th><th className="py-3.5 px-4 text-center">Estado</th><th className="py-3.5 px-4 text-center">Acciones</th></tr></thead><tbody className="divide-y divide-slate-200 dark:divide-slate-800">{initialUsers.map(item => { const isSelf = item.id === currentUserId; const canSuspend = !isSelf && item.role === "COLLABORATOR" && item.status !== "SUSPENDED"; return <tr key={item.id}><td className="py-4 px-4 font-bold text-slate-950 dark:text-white">{item.name || "Sin nombre"}</td><td className="py-4 px-4 text-slate-700 dark:text-slate-300">{item.email}</td><td className="py-4 px-4 text-center"><span className="rounded-full border px-2.5 py-1 text-[9px] font-bold">{roleLabel(item.role)}</span></td><td className="py-4 px-4 text-center font-bold">{item._count.cards}</td><td className="py-4 px-4 text-center text-xs">{item.status === "PENDING" ? "Pendiente de activación" : item.status === "ACTIVE" ? "Activo" : item.status === "SUSPENDED" ? "Suspendido" : item.status}</td><td className="py-4 px-4 text-center">{isSelf ? <span className="text-xs text-slate-500 italic">Tu cuenta</span> : <div className="flex justify-center gap-2">{item.status === "PENDING" && <button disabled={isPending} onClick={() => handleResend(item.id, item.email)} className="rounded-lg bg-amber-500/10 px-3 py-1.5 text-xs font-semibold text-amber-600">✉️ Reenviar</button>}{canSuspend && <button disabled={isPending} onClick={() => handleSuspend(item.id, item.name || item.email)} className="rounded-lg bg-rose-500/10 px-3 py-1.5 text-xs font-semibold text-rose-600">⏸️ Suspender</button>}{item.status === "SUSPENDED" && <span className="rounded-lg bg-slate-500/10 px-3 py-1.5 text-xs font-semibold text-slate-500">Historial conservado</span>}</div>}</td></tr>; })}</tbody></table></div></div>
