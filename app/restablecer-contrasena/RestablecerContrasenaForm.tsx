...
import { completePasswordResetAction } from "../superadmin/actions";
import SmartNFCLogo from "../../components/brand/SmartNFCLogo";

interface RestablecerContrasenaFormProps {
  token: string;
  userName: string;
}
...
export default function RestablecerContrasenaForm({
  token,
  userName,
}: RestablecerContrasenaFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await completePasswordResetAction(token, password);
      setSuccess(true);
      setTimeout(() => {
      ...
      <div className="max-w-md w-full bg-white dark:bg-slate-900 border border-slate-200/50 dark:border-white/5 p-6 sm:p-8 rounded-2xl shadow-2xl space-y-6">
        ...
        <div className="text-center space-y-3">
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400 font-medium">
            Hola <strong className="text-slate-800 dark:text-slate-200">{userName}</strong>, ingresa tu nueva contraseña segura.
          </p>
        </div>
        ...
        {success ? (
          <div className="p-4 bg-emerald-500/10 border border-emerald-500/20 text-emerald-600 dark:text-emerald-400 text-xs font-semibold rounded-xl text-center space-y-2">
            <div>🎉 ¡Contraseña restablecida con éxito!</div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="space-y-1">
              ...
              <input
                ...
                placeholder="••••••••"
                className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200/50 dark:border-slate-800 rounded-xl px-4 py-2.5 text-xs outline-none focus:ring-1 focus:ring-blue-600 dark:text-white"
              />
            </div>
            ...
            <div className="p-4 bg-slate-50 dark:bg-slate-950 border border-slate-200/50 dark:border-slate-850 rounded-xl text-[10px] space-y-1.5 text-slate-500 dark:text-slate-450">
              <span className="font-bold block uppercase tracking-wider text-slate-400 dark:text-slate-550 mb-1 text-[9px]">
