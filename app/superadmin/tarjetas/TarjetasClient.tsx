...
export default function TarjetasClient({ cards: initialCards, companies, digitalCards, companyUsers, originHost }: TarjetasClientProps) {
  ...
  const handleDisassociate = async (cardId: string, token: string) => {
    if (!window.confirm(`¿Estás seguro de desvincular el destino de la tarjeta "${token.substring(0, 12)}..."?\n\nLa tarjeta continuará perteneciendo a su empresa original y quedará sin destino asociado.`)) return;
    startTransition(async () => {
  ...
  return <div className="space-y-6">
    ...
    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4"><div><h1 className="text-2xl font-black tracking-tight text-slate-900 dark:text-white">Inventario de Tarjetas Físicas NFC</h1><p className="mt-1.5 text-xs text-slate-700 dark:text-slate-300 font-medium">Cada tarjeta física queda asociada permanentemente a la empresa para la cual fue producida.</p></div><button onClick={openRegisterModal} className="bg-blue-600 hover:bg-blue-500 text-white text-xs font-extrabold px-4 py-2.5 rounded-xl shadow-md">➕ Registrar Nueva Tarjeta</button></div>
