...
type PhysicalCardItem = {
  id: string; token: string; uid: string | null; status: string; batchCode: string | null; createdAt: Date;
  companyId: string; cardId: string | null; localTouchpointId: string | null;
...
export default function TarjetasClient({ cards: initialCards, companies, digitalCards, companyUsers, originHost }: TarjetasClientProps) {
  ...
  const handleReassignCompany = async (cardId: string) => {
    ...
    if (!card || card.cardId || card.localTouchpointId) { setErrorMsg("Primero debes desvincular el destino antes de cambiar la empresa de una tarjeta física."); setReassigningCardId(null); return; }
    startTransition(async () => {
