...
export default function ReportSummary({report}:{report:ReportSnapshot}) {
  ...
  const cards=[["Accesos",m.visits],["NFC",m.nfc],["QR",m.qr],["Directos",m.direct],
    ...
    ["Conversión",m.conversionRate.toFixed(1)+"%"],["Clics WhatsApp",m.whatsapp]];
  return <section className="space-y-6">
    ...
    <p className="text-sm text-slate-500">Un acceso es una visita, no una persona única. Conversión: visitas con registro ÷ accesos del mismo periodo. Los clics no confirman mensajes enviados. Cero actividad registrada no acredita ausencia de visitantes. Estos informes usan la medición corregida; el historial anterior se conserva.</p>
  </section>;
}
