...
async function metrics(tx:Prisma.TransactionClient,companyId:string,start:Date,end:Date):Promise<ReportMetrics> {
  ...
  const [sources,points,conversions,newSubscribers,events,converted,clicks] = await Promise.all([
    tx.localVisit.groupBy({by:["source"],where,_count:{_all:true}}),
    tx.localVisit.groupBy({by:["touchpointId"],where,_count:{_all:true}}),
    tx.localVisit.count({where:{...where,events:{some:{eventType:"SUBSCRIPTION",createdAt:{lt:end}}}}}),
    ...
    tx.localVisit.groupBy({by:["touchpointId"],where:{...where,events:{some:{eventType:"SUBSCRIPTION",createdAt:{lt:end}}}},_count:{_all:true}}),
  ...
  return {visits,nfc:count("NFC"),qr:count("QR"),direct:count("DIRECT")+count("UNKNOWN"),
    conversions,conversionRate:visits?100*conversions/visits:0,newSubscribers,
