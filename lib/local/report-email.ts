...
export function renderReportEmail(snapshot:ReportSnapshot,url:string) {
  ...
  ["Visitas con registro",m.conversions],["Conversión del Club",m.conversionRate.toFixed(1)+"%"],["Clics a WhatsApp",m.whatsapp],
  ...
  ${m.objectives?.length?`<h2 style="font-size:18px">Accesos por objetivo</h2>${m.objectives.map(item=>`<p>${escapeHtml(item.label)}: ${item.visits}</p>`).join("")}<p>Salidas a destinos: ${m.destinationRedirects || 0}. No confirman reseñas, seguidores ni compras.</p>`:""}
  <h2 style="font-size:18px">Tus puntos</h2>
